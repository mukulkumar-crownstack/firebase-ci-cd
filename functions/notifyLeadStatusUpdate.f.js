...
exports.notifyLeadStatusUpdate = ((change, context) => {
    ...
    const translation = newValue.phone_country_code === "us" ? constants_1.en : constants_1.es;
    const driverType = translation[helper_functions_1.getDriverType(newValue.driver_type_code, newValue.phone_country_code)];
    const vehicleType = helper_functions_1.getVehicleType(newValue.vehicle_type_codes, newValue.phone_country_code);
    const { full_name: name, phone, operating_city: { city } = '', application_id: applicationID, lead_status: leadStatus, application_status: applicationStatus } = newValue;
    ...
    let slackText;
    let smsText;
    if (newValue.lead_status !== previousValue.lead_status
        || newValue.application_status !== previousValue.application_status) {
        if (leadStatus === enums_1.LeadStatus.APPLIED) {
            smsText = `${translation['Thank you for applying with Partrunner. Your application ID is :applicationID .\n\nIf you want to continue or make changes, click here'].replace(':applicationID', applicationID)} ${helper_functions_1.getPartrunnerBaseURL('driver')}/${translation.language}/login`;
        }
        ...
        `;
                ...
                const available = helper_functions_1.getDriverAvailablity(newValue.availability_of_driver, newValue.phone_country_code);
                slackText = `
          ${translation["A new driver has been approved"]}
        ...
        if (slackText) {
            helper_functions_1.sendSlackNotification(slackText, newValue.phone_country_code);
        }
    }
});
